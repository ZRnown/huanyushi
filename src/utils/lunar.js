; (function (root, factory) {
    ...
    var LunarUtil = (function () {
        return {
            ...
            GAN_TO_TEMPLATE: { '甲': '{tg.jia}', '乙': '{tg.yi}', '丙': '{tg.bing}', '丁': '{tg.ding}', '戊': '{tg.wu}', '己': '{tg.ji}', '庚': '{tg.geng}', '辛': '{tg.xin}', '壬': '{tg.ren}', '癸': '{tg.gui}' },
            ZHI_TO_TEMPLATE: { '子': '{dz.zi}', '丑': '{dz.chou}', '寅': '{dz.yin}', '卯': '{dz.mao}', '辰': '{dz.chen}', '巳': '{dz.si}', '午': '{dz.wu}', '未': '{dz.wei}', '申': '{dz.shen}', '酉': '{dz.you}', '戌': '{dz.xu}', '亥': '{dz.hai}' },
            SHI_SHEN_TEMPLATE_TO_SHORT: {
                '{ss.biJian}': '比',
                '{ss.jieCai}': '劫',
                '{ss.shiShen}': '食',
                '{ss.shangGuan}': '伤',
    ...
    var EightChar = (function () {
        var _fromLunar = function (lunar, gender) {
            return {
                _p: { sect: 2, lunar: lunar, gender: gender, shenShaCache: null },
                // 获取勾绞煞
                _getGouJiaoSha: function () {
                    var yearZhiIndex = this._p.lunar.getYearZhiIndexExact();
                    var isYangYear = this._p.lunar.getYearGanIndexExact() % 2 === 0;
                    var isMale = this._p.gender === 1;
                    var gouZhiIndex = (yearZhiIndex + 3) % 12;
                    var jiaoZhiIndex = (yearZhiIndex - 3 + 12) % 12;

                    var gouZhi = LunarUtil.ZHI[gouZhiIndex + 1];
                    var jiaoZhi = LunarUtil.ZHI[jiaoZhiIndex + 1];
                    ...
                    return { gou: gou, jiao: jiao };
                },

                _calculateShenSha: function () {
                    // 如果已有缓存，直接返回
                    if (this._p.shenShaCache) return this._p.shenShaCache;

                    // 初始化神煞存储对象
                    var shens = { year: [], month: [], day: [], time: [] };
                    var pillars = ['year', 'month', 'day', 'time'];

                    // 获取四柱天干地支
                    var allStems = [this.getYearGan(), this.getMonthGan(), this.getDayGan(), this.getTimeGan()];
                    var allZhis = [this.getYearZhi(), this.getMonthZhi(), this.getDayZhi(), this.getTimeZhi()];

                    // 常用干支变量
                    var dayGan = this.getDayGan();
                    var yearGan = this.getYearGan();
                    var dayZhi = this.getDayZhi();
                    var yearZhi = this.getYearZhi();
                    var monthZhi = this.getMonthZhi();
                    var timeZhi = this.getTimeZhi();
                    var dayPillar = this.getDay();
                    var timePillar = this.getTime();
                    var lunarMonth = this._p.lunar.getMonth();

                    // 辅助函数：向指定柱添加神煞
                    var addShenSha = function (pillarIndex, name) {
                        if (pillarIndex < 0 || pillarIndex > 3) return;
                        if (shens[pillars[pillarIndex]]) {
                            shens[pillars[pillarIndex]].push(name);
                        }
                    };

                    // 辅助函数：在四柱地支中查找目标并添加神煞
                    var findShenShaInZhi = function (targets, name) {
                        allZhis.forEach(function (z, i) {
                            if (targets.includes(z)) addShenSha(i, name);
                        });
                    };
                    ...
                    // 天乙贵人
                    var tianYiRule = { '甲': ['丑', '未'], '戊': ['丑', '未'], '庚': ['丑', '未'], '乙': ['子', '申'], '己': ['子', '申'], '丙': ['亥', '酉'], '丁': ['亥', '酉'], '壬': ['卯', '巳'], '癸': ['卯', '巳'], '辛': ['寅', '午'] };
                    var tianYiTargets = (tianYiRule[dayGan] || []).concat((tianYiRule[yearGan] || []).filter(function (item) { return (tianYiRule[dayGan] || []).indexOf(item) < 0; }));
                    if (tianYiTargets.length > 0) findShenShaInZhi(tianYiTargets, '天乙贵人');

                    // 太极贵人
                    var taiJiRule = { '甲': ['子', '午'], '乙': ['子', '午'], '丙': ['酉', '卯'], '丁': ['酉', '卯'], '戊': ['辰', '戌', '丑', '未'], '己': ['辰', '戌', '丑', '未'], '庚': ['寅', '亥'], '辛': ['寅', '亥'], '壬': ['巳', '申'], '癸': ['巳', '申'] };
                    var taiJiTargets = (taiJiRule[dayGan] || []).concat((taiJiRule[yearGan] || []).filter(function (item) { return (taiJiRule[dayGan] || []).indexOf(item) < 0; }));
                    if (taiJiTargets.length > 0) findShenShaInZhi(taiJiTargets, '太极贵人');

                    // 天德/月德贵人
                    var tianDeRule = { 1: '丁', 2: '申', 3: '壬', 4: '辛', 5: '亥', 6: '甲', 7: '癸', 8: '寅', 9: '丙', 10: '乙', 11: '巳', 12: '庚' };
                    var tianDeTarget = tianDeRule[lunarMonth < 0 ? -lunarMonth : lunarMonth];
                    if (tianDeTarget) {
                        allStems.forEach(function (s, i) { if (s === tianDeTarget) addShenSha(i, '天德贵人'); });
                        allZhis.forEach(function (z, i) { if (z === tianDeTarget) addShenSha(i, '天德贵人'); });
                    }
                    var yueDeTarget = '';
                    if (['寅', '午', '戌'].includes(monthZhi)) yueDeTarget = '丙';
                    else if (['申', '子', '辰'].includes(monthZhi)) yueDeTarget = '壬';
                    else if (['亥', '卯', '未'].includes(monthZhi)) yueDeTarget = '甲';
                    else if (['巳', '酉', '丑'].includes(monthZhi)) yueDeTarget = '庚';
                    if (yueDeTarget) {
                        allStems.forEach(function (s, i) { if (s === yueDeTarget) addShenSha(i, '月德贵人'); });
                    }

                    // 三奇贵人
                    if (allStems[0] === '乙' && allStems[1] === '丙' && allStems[2] === '丁') { addShenSha(0, '三奇贵人'); addShenSha(1, '三奇贵人'); addShenSha(2, '三奇贵人'); }
                    if (allStems[1] === '乙' && allStems[2] === '丙' && allStems[3] === '丁') { addShenSha(1, '三奇贵人'); addShenSha(2, '三奇贵人'); addShenSha(3, '三奇贵人'); }
                    if (allZhis[0] === '卯' && allZhis[1] === '巳' && allZhis[2] === '午') { addShenSha(0, '三奇贵人'); addShenSha(1, '三奇贵人'); addShenSha(2, '三奇贵人'); }
                    if (allZhis[1] === '卯' && allZhis[2] === '巳' && allZhis[3] === '午') { addShenSha(1, '三奇贵人'); addShenSha(2, '三奇贵人'); addShenSha(3, '三奇贵人'); }

                    // 福星贵人
                    var fuXingRule = { '甲': ['寅', '子'], '丙': ['寅', '子'], '乙': ['卯', '丑'], '癸': ['卯', '丑'], '戊': ['申'], '己': ['未'], '丁': ['亥'], '庚': ['午'], '辛': ['巳'], '壬': ['辰'] };
                    var fuXingTargets = (fuXingRule[dayGan] || []).concat((fuXingRule[yearGan] || []).filter(function (item) { return (fuXingRule[dayGan] || []).indexOf(item) < 0; }));
                    if (fuXingTargets.length > 0) findShenShaInZhi(fuXingTargets, '福星贵人');

                    // 文昌贵人
                    var wenChangRule = { '甲': ['巳'], '乙': ['午'], '丙': ['申'], '戊': ['申'], '丁': ['酉'], '己': ['酉'], '庚': ['亥'], '辛': ['子'], '壬': ['寅'], '癸': ['卯'] };
                    var wenChangTargets = (wenChangRule[dayGan] || []).concat((wenChangRule[yearGan] || []).filter(function (item) { return (wenChangRule[dayGan] || []).indexOf(item) < 0; }));
                    if (wenChangTargets.length > 0) findShenShaInZhi(wenChangTargets, '文昌贵人');

                    // 天厨贵人
                    var tianChuRule = { '甲': '巳', '乙': '午', '丙': '子', '丁': '巳', '戊': '午', '己': '申', '庚': '寅', '辛': '午', '壬': '酉', '癸': '亥' };
                    var tianChuTargets1 = tianChuRule[dayGan] ? [tianChuRule[dayGan]] : [];
                    var tianChuTargets2 = tianChuRule[yearGan] ? [tianChuRule[yearGan]] : [];
                    var tianChuTargets = tianChuTargets1.concat(tianChuTargets2.filter(function (item) { return tianChuTargets1.indexOf(item) < 0; }));
                    if (tianChuTargets.length > 0) findShenShaInZhi(tianChuTargets, '天厨贵人');

                    // 魁罡贵人
                    if (['壬辰', '庚戌', '庚辰', '戊戌'].includes(dayPillar)) addShenSha(2, '魁罡贵人');

                    // 国印贵人
                    var guoYinRule = { '甲': ['戌'], '乙': ['亥'], '丙': ['丑'], '丁': ['寅'], '戊': ['丑'], '己': ['寅'], '庚': ['辰'], '辛': ['巳'], '壬': ['未'], '癸': ['申'] };
                    var guoYinTargets = (guoYinRule[dayGan] || []).concat((guoYinRule[yearGan] || []).filter(function (item) { return (guoYinRule[dayGan] || []).indexOf(item) < 0; }));
                    if (guoYinTargets.length > 0) findShenShaInZhi(guoYinTargets, '国印贵人');

                    // 学堂 & 词馆
                    var xueTangRule = { '金': '巳', '木': '亥', '水': '申', '土': '申', '火': '寅' };
                    var ciGuanRule = { '甲': '寅', '乙': '卯', '丙': '巳', '丁': '午', '戊': '巳', '己': '午', '庚': '申', '辛': '酉', '壬': '亥', '癸': '子' };
                    var yearNaYinWuXing = this.getYearNaYin().slice(-1);
                    var dayNaYinWuXing = this.getDayNaYin().slice(-1);
                    var xueTangTargets = [xueTangRule[yearNaYinWuXing], xueTangRule[dayNaYinWuXing]].filter(Boolean);
                    var ciGuanTargets = [ciGuanRule[yearGan], ciGuanRule[dayGan]].filter(Boolean);
                    allZhis.forEach(function (z, i) {
                        if (xueTangTargets.includes(z)) addShenSha(i, '学堂');
                        if (ciGuanTargets.includes(z)) addShenSha(i, '词馆');
                    });

                    // 德秀贵人 (逻辑正确，无需修改)
                    var deXiuDeRule = [], deXiuXiuRule = [];
                    if (['寅', '午', '戌'].includes(monthZhi)) { deXiuDeRule = ['丙', '丁']; deXiuXiuRule = ['戊', '癸']; }
                    else if (['申', '子', '辰'].includes(monthZhi)) { deXiuDeRule = ['壬', '癸', '戊', '己']; deXiuXiuRule = ['丙', '辛', '甲', '己']; }
                    else if (['巳', '酉', '丑'].includes(monthZhi)) { deXiuDeRule = ['庚', '辛']; deXiuXiuRule = ['乙', '庚']; }
                    else if (['亥', '卯', '未'].includes(monthZhi)) { deXiuDeRule = ['甲', '乙']; deXiuXiuRule = ['丁', '壬']; }
                    var hasDe = allStems.some(function (s) { return deXiuDeRule.includes(s); });
                    var hasXiu = allStems.some(function (s) { return deXiuXiuRule.includes(s); });
                    if (hasDe && hasXiu) {
                        allStems.forEach(function (s, i) {
                            if (deXiuDeRule.includes(s) || deXiuXiuRule.includes(s)) addShenSha(i, '德秀贵人');
                        });
                    }

                    // 驿马, 华盖, 将星 (逻辑正确，无需修改)
                    var sanHeRules = { '驿马': { '申子辰': '寅', '寅午戌': '申', '巳酉丑': '亥', '亥卯未': '巳' }, '华盖': { '寅午戌': '戌', '亥卯未': '未', '申子辰': '辰', '巳酉丑': '丑' }, '将星': { '寅午戌': '午', '巳酉丑': '酉', '申子辰': '子', '亥卯未': '卯' } };
                    Object.keys(sanHeRules).forEach(function (shen) {
                        var rule = sanHeRules[shen];
                        Object.keys(rule).forEach(function (group) {
                            if (group.includes(yearZhi)) allZhis.forEach(function (z, i) { if (z === rule[group]) addShenSha(i, shen); });
                            if (group.includes(dayZhi)) allZhis.forEach(function (z, i) { if (z === rule[group]) addShenSha(i, shen); });
                        });
                    });

                    // 金舆
                    var jinYuRule = { '甲': '辰', '乙': '巳', '丙': '未', '丁': '申', '戊': '未', '己': '申', '庚': '戌', '辛': '亥', '壬': '丑', '癸': '寅' };
                    var jinYuTargets = (jinYuRule[dayGan] ? [jinYuRule[dayGan]] : []).concat((jinYuRule[yearGan] ? [jinYuRule[yearGan]] : []).filter(function (item) { return (jinYuRule[dayGan] ? [jinYuRule[dayGan]] : []).indexOf(item) < 0; }));
                    if (jinYuTargets.length > 0) findShenShaInZhi(jinYuTargets, '金舆');

                    // 金神
                    if (['乙丑', '己巳', '癸酉'].includes(dayPillar)) addShenSha(2, '金神');
                    if (['乙丑', '己巳', '癸酉'].includes(timePillar)) addShenSha(3, '金神');

                    // 天医
                    var tianYiTarget = LunarUtil.ZHI[(this._p.lunar.getMonthZhiIndex() - 1 + 12) % 12 + 1];
                    if (tianYiTarget) findShenShaInZhi([tianYiTarget], '天医');

                    // 禄神
                    var luShenRule = { '甲': '寅', '乙': '卯', '丙': '巳', '丁': '午', '戊': '巳', '己': '午', '庚': '申', '辛': '酉', '壬': '亥', '癸': '子' };
                    var luShenTarget = luShenRule[dayGan];
                    if (luShenTarget) findShenShaInZhi([luShenTarget], '禄神');

                    // 拱禄
                    var gongLuRule = { '癸亥日癸丑时': '子', '癸丑日癸亥时': '子', '丁巳日丁未时': '午', '己未日己巳时': '午', '戊辰日戊午时': '巳' };
                    if (gongLuRule[dayPillar + '日' + timePillar + '时']) { addShenSha(2, '拱禄'); addShenSha(3, '拱禄'); }

                    // 天赦
                    if ((['寅', '卯', '辰'].includes(monthZhi) && dayPillar === '戊寅') ||
                        (['巳', '午', '未'].includes(monthZhi) && dayPillar === '甲午') ||
                        (['申', '酉', '戌'].includes(monthZhi) && dayPillar === '戊申') ||
                        (['亥', '子', '丑'].includes(monthZhi) && dayPillar === '甲子')) {
                        addShenSha(2, '天赦');
                    }

                    // 日德, 日贵, 六秀, 十灵日, 进神
                    if (['甲寅', '丙辰', '戊辰', '庚辰', '壬戌'].includes(dayPillar)) addShenSha(2, '日德');
                    if (['丁酉', '丁亥', '癸卯', '癸巳'].includes(dayPillar)) addShenSha(2, '日贵');
                    if (['丙午', '丁未', '戊子', '戊午', '己丑', '己未'].includes(dayPillar)) addShenSha(2, '六秀');
                    if (['甲辰', '乙亥', '丙辰', '丁酉', '戊午', '庚戌', '庚寅', '辛亥', '壬寅', '癸未'].includes(dayPillar)) addShenSha(2, '十灵日');
                    if (['甲子', '甲午', '己卯', '己酉'].includes(dayPillar)) addShenSha(2, '进神');

                    // --- 凶煞 ---

                    // 天罗地网
                    var yearNaYinWuXing = this.getYearNaYin().slice(-1);
                    if (yearNaYinWuXing === '火' && (yearZhi === '戌' || yearZhi === '亥')) addShenSha(0, '天罗');
                    if ((yearNaYinWuXing === '水' || yearNaYinWuXing === '土') && (yearZhi === '辰' || yearZhi === '巳')) addShenSha(0, '地网');
                    if (yearZhi === '辰' && allZhis.includes('巳')) { addShenSha(0, '地网'); allZhis.forEach(function (z, i) { if (z === '巳') addShenSha(i, '地网'); }); }
                    if (yearZhi === '巳' && allZhis.includes('辰')) { addShenSha(0, '地网'); allZhis.forEach(function (z, i) { if (z === '辰') addShenSha(i, '地网'); }); }
                    if (yearZhi === '戌' && allZhis.includes('亥')) { addShenSha(0, '天罗'); allZhis.forEach(function (z, i) { if (z === '亥') addShenSha(i, '天罗'); }); }
                    if (yearZhi === '亥' && allZhis.includes('戌')) { addShenSha(0, '天罗'); allZhis.forEach(function (z, i) { if (z === '戌') addShenSha(i, '天罗'); }); }

                    // 羊刃
                    var yangRenRule = { '甲': '卯', '乙': '寅', '丙': '午', '丁': '巳', '戊': '午', '己': '巳', '庚': '酉', '辛': '申', '壬': '子', '癸': '亥' };
                    var yangRenTarget = yangRenRule[dayGan];
                    if (yangRenTarget) findShenShaInZhi([yangRenTarget], '羊刃');

                    // 亡神, 劫煞, 灾煞
                    var xiongShaRules = { '亡神': { '寅午戌': '巳', '亥卯未': '寅', '巳酉丑': '申', '申子辰': '亥' }, '劫煞': { '申子辰': '巳', '亥卯未': '申', '寅午戌': '亥', '巳酉丑': '寅' }, '灾煞': { '申子辰': '午', '亥卯未': '酉', '寅午戌': '子', '巳酉丑': '卯' } };
                    Object.keys(xiongShaRules).forEach(function (shen) {
                        var rule = xiongShaRules[shen];
                        Object.keys(rule).forEach(function (group) {
                            if (group.includes(yearZhi)) allZhis.forEach(function (z, i) { if (z === rule[group]) addShenSha(i, shen); });
                            if (shen !== '灾煞' && group.includes(dayZhi)) allZhis.forEach(function (z, i) { if (z === rule[group]) addShenSha(i, shen); });
                        });
                    });

                    // 勾绞煞
                    var gouJiao = this._getGouJiaoSha();
                    allZhis.forEach(function (z, i) {
                        if (z === gouJiao.gou) addShenSha(i, '勾煞');
                        if (z === gouJiao.jiao) addShenSha(i, '绞煞');
                    });

                    // 孤辰寡宿
                    var guChenTarget = '', guaSuTarget = '';
                    if (['亥', '子', '丑'].includes(yearZhi)) { guChenTarget = '寅'; guaSuTarget = '戌'; }
                    else if (['寅', '卯', '辰'].includes(yearZhi)) { guChenTarget = '巳'; guaSuTarget = '丑'; }
                    else if (['巳', '午', '未'].includes(yearZhi)) { guChenTarget = '申'; guaSuTarget = '辰'; }
                    else if (['申', '酉', '戌'].includes(yearZhi)) { guChenTarget = '亥'; guaSuTarget = '未'; }
                    allZhis.forEach(function (z, i) {
                        if (z === guChenTarget) addShenSha(i, '孤辰');
                        if (z === guaSuTarget) addShenSha(i, '寡宿');
                    });

                    // 阴阳差错, 孤鸾煞, 十恶大败
                    if (['丙子', '丁丑', '戊寅', '辛卯', '壬辰', '癸巳', '丙午', '丁未', '戊申', '辛酉', '壬戌', '癸亥'].includes(dayPillar)) addShenSha(2, '阴阳差错');
                    if (['乙巳', '丁巳', '辛亥', '戊申', '壬寅', '戊午', '壬子', '丙午'].includes(dayPillar) || ['乙巳', '丁巳', '辛亥', '戊申', '壬寅', '戊午', '壬子', '丙午'].includes(timePillar)) {
                        if (['乙巳', '丁巳', '辛亥', '戊申', '壬寅', '戊午', '壬子', '丙午'].includes(dayPillar)) addShenSha(2, '孤鸾煞');
                        if (['乙巳', '丁巳', '辛亥', '戊申', '壬寅', '戊午', '壬子', '丙午'].includes(timePillar)) addShenSha(3, '孤鸾煞');
                    }
                    if (['甲辰', '乙巳', '壬申', '丙申', '丁亥', '庚辰', '戊戌', '癸亥', '辛巳', '己丑'].includes(dayPillar)) addShenSha(2, '十恶大败');

                    // 四废
                    if ((['寅', '卯', '辰'].includes(monthZhi) && ['庚申', '辛酉'].includes(dayPillar)) ||
                        (['巳', '午', '未'].includes(monthZhi) && ['壬子', '癸亥'].includes(dayPillar)) ||
                        (['申', '酉', '戌'].includes(monthZhi) && ['甲寅', '乙卯'].includes(dayPillar)) ||
                        (['亥', '子', '丑'].includes(monthZhi) && ['丙午', '丁巳'].includes(dayPillar))) {
                        addShenSha(2, '四废');
                    }
                    ...
                    var isYangYear = this._p.lunar.getYearGanIndexExact() % 2 === 0;
                    var isMale = this._p.gender === 1;
                    var yuanChenRule = (isYangYear && isMale) || (!isYangYear && !isMale) ?
                        { '子': '未', '丑': '申', '寅': '酉', '卯': '戌', '辰': '亥', '巳': '子', '午': '丑', '未': '寅', '申': '卯', '酉': '辰', '戌': '巳', '亥': '午' } :
                        { '子': '巳', '丑': '午', '寅': '未', '卯': '申', '辰': '酉', '巳': '戌', '午': '亥', '未': '子', '申': '丑', '酉': '寅', '戌': '卯', '亥': '辰' };
                    var yuanChenTarget = yuanChenRule[yearZhi];
                    if (yuanChenTarget) findShenShaInZhi([yuanChenTarget], '元辰');

                    // 空亡
                    var xunKong = this.getDayXunKong();
                    if (xunKong) {
                        var kongWangTargets = xunKong.split('');
                        if (kongWangTargets.includes(yearZhi)) addShenSha(0, '空亡');
                        if (kongWangTargets.includes(monthZhi)) addShenSha(1, '空亡');
                        if (kongWangTargets.includes(timeZhi)) addShenSha(3, '空亡');
                    }

                    // 桃花/红艳煞 (名称已修改)
                    var taoHuaRule = { '申子辰': '酉', '寅午戌': '卯', '巳酉丑': '午', '亥卯未': '子' };
                    Object.keys(taoHuaRule).forEach(function (group) {
                        if (group.includes(yearZhi)) findShenShaInZhi([taoHuaRule[group]], '桃花');
                        if (group.includes(dayZhi)) findShenShaInZhi([taoHuaRule[group]], '桃花');
                    });
                    var hongYanRule = { '甲': '午', '乙': '午', '丙': '寅', '丁': '未', '戊': '辰', '己': '辰', '庚': '戌', '辛': '酉', '壬': '子', '癸': '申' };
                    var hongYanTarget = hongYanRule[dayGan];
                    if (hongYanTarget) findShenShaInZhi([hongYanTarget], '红艳煞');

                    // 红鸾, 天喜
                    var hongLuanRule = { '子': '卯', '丑': '寅', '寅': '丑', '卯': '子', '辰': '亥', '巳': '戌', '午': '酉', '未': '申', '申': '未', '酉': '午', '戌': '巳', '亥': '辰' };
                    var hongLuanTarget = hongLuanRule[yearZhi];
                    if (hongLuanTarget) {
                        var tianXiTarget = LunarUtil.ZHI[(LunarUtil.ZHI.indexOf(hongLuanTarget) + 6) % 12];
                        findShenShaInZhi([hongLuanTarget], '红鸾');
                        findShenShaInZhi([tianXiTarget], '天喜');
                    }

                    // 八专, 九丑
                    if (['甲寅', '乙卯', '己未', '丁未', '庚申', '辛酉', '戊戌', '癸丑'].includes(dayPillar)) addShenSha(2, '八专');
                    if (['戊子', '戊午', '壬子', '壬午', '乙卯', '辛卯', '己卯', '乙酉', '辛酉', '己酉'].includes(dayPillar)) addShenSha(2, '九丑');

                    // 血支，血刃，流霞
                    var xueZhiRule = { '子': '戌', '丑': '酉', '寅': '申', '卯': '未', '辰': '午', '巳': '巳', '午': '辰', '未': '卯', '申': '寅', '酉': '丑', '戌': '子', '亥': '亥' };
                    var xueRenRule = { 1: '丑', 2: '未', 3: '寅', 4: '申', 5: '卯', 6: '酉', 7: '辰', 8: '戌', 9: '巳', 10: '亥', 11: '午', 12: '子' };
                    var liuXiaRule = { '甲': '酉', '乙': '戌', '丙': '未', '丁': '申', '戊': '巳', '己': '午', '庚': '辰', '辛': '卯', '壬': '亥', '癸': '寅' };
                    if (xueZhiRule[yearZhi]) findShenShaInZhi([xueZhiRule[yearZhi]], '血支');
                    if (xueRenRule[lunarMonth < 0 ? -lunarMonth : lunarMonth]) findShenShaInZhi([xueRenRule[lunarMonth < 0 ? -lunarMonth : lunarMonth]], '血刃');
                    if (liuXiaRule[dayGan]) findShenShaInZhi([liuXiaRule[dayGan]], '流霞');

                    // **[修正]** 丧门, 吊客, 披麻
                    var yearZhiIndex = LunarUtil.ZHI.indexOf(yearZhi);
                    var sangMenTarget = LunarUtil.ZHI[(yearZhiIndex + 2) % 12];
                    var diaoKeTarget = LunarUtil.ZHI[(yearZhiIndex - 2 + 12) % 12];
                    // 披麻根据您提供的对照表进行查找
                    var piMaRule = { '子': '酉', '丑': '戌', '寅': '亥', '卯': '子', '辰': '丑', '巳': '寅', '午': '卯', '未': '辰', '申': '巳', '酉': '午', '戌': '未', '亥': '申' };
                    var piMaTarget = piMaRule[yearZhi];
                    allZhis.forEach(function (z, i) {
                        if (z === sangMenTarget) addShenSha(i, '丧门');
                        if (z === diaoKeTarget) addShenSha(i, '吊客');
                        if (piMaTarget && z === piMaTarget) addShenSha(i, '披麻');
                    });

                    // 童子煞
                    var springAutumn = ['寅', '卯', '辰', '申', '酉', '戌'];
                    var isTongZi = false;
                    if (springAutumn.includes(monthZhi)) {
                        if (['寅', '子'].includes(dayZhi)) { addShenSha(2, '童子煞'); isTongZi = true; }
                        if (['寅', '子'].includes(timeZhi)) { addShenSha(3, '童子煞'); isTongZi = true; }
                    } else { // 冬夏季
                        if (['卯', '未', '辰'].includes(dayZhi)) { addShenSha(2, '童子煞'); isTongZi = true; }
                        if (['卯', '未', '辰'].includes(timeZhi)) { addShenSha(3, '童子煞'); isTongZi = true; }
                    }
                    if (!isTongZi) {
                        var yearNaYinWuXing = this.getYearNaYin().slice(-1);
                        if (['金', '木'].includes(yearNaYinWuXing)) {
                            if (['午', '卯'].includes(dayZhi)) { addShenSha(2, '童子煞'); }
                            if (['午', '卯'].includes(timeZhi)) { addShenSha(3, '童子煞'); }
                        } else if (['水', '火'].includes(yearNaYinWuXing)) {
                            if (['酉', '戌'].includes(dayZhi)) { addShenSha(2, '童子煞'); }
                            if (['酉', '戌'].includes(timeZhi)) { addShenSha(3, '童子煞'); }
                        } else if (yearNaYinWuXing === '土') {
                            if (['辰', '巳'].includes(dayZhi)) { addShenSha(2, '童子煞'); }
                            if (['辰', '巳'].includes(timeZhi)) { addShenSha(3, '童子煞'); }
                        }
                    }

                    // 去重，确保每个柱的神煞不重复
                    Object.keys(shens).forEach(function (key) {
                        shens[key] = shens[key].filter(function (value, index, self) { return self.indexOf(value) === index; });
                    });

                    // 缓存结果
                    this._p.shenShaCache = shens;
                    return shens;
                },

                getYearShenSha: function () {
                    return this._calculateShenSha().year;
                },
                getMonthShenSha: function () {
                    return this._calculateShenSha().month;
                },
                getDayShenSha: function () {
                    return this._calculateShenSha().day;
                },
                getTimeShenSha: function () {
                    return this._calculateShenSha().time;
                },
                ...
                _getChangShengState: function (gan, ganIndex, zhiIndex) {
                    // 获取该天干的长生起始点偏移量
                    var offset = LunarUtil.CHANG_SHENG_OFFSET[gan];
                    // 判断天干阴阳，阳干顺行，阴干逆行
                    var isYang = ganIndex % 2 === 0;
                    var index = offset + (isYang ? zhiIndex : -zhiIndex);
                    // 修正索引使其落在 0-11 的范围内
                    if (index >= 12) {
                        index -= 12;
                    }
                ...
                // 年柱自坐
                getYearZuo: function () {
                    return this._getChangShengState(this.getYearGan(), this._p.lunar.getYearGanIndexExact(), this._p.lunar.getYearZhiIndexExact());
                },
                ...
                // 月柱自坐
                getMonthZuo: function () {
                    return this._getChangShengState(this.getMonthGan(), this._p.lunar.getMonthGanIndexExact(), this._p.lunar.getMonthZhiIndexExact());
                },
                ...
                getDayShiShenGan: function () { return this._p.gender === 1 ? '元男' : '元女'; },
                ...
                // 日柱自坐
                getDayZuo: function () {
                    // 对于日柱来说，“自坐”的结果和原来的“日干地势”是一样的
                    return this._getChangShengState(this.getDayGan(), this.getDayGanIndex(), this.getDayZhiIndex());
                },
                ...
                // 时柱自坐
                getTimeZuo: function () {
                    return this._getChangShengState(this.getTimeGan(), this._p.lunar.getTimeGanIndex(), this._p.lunar.getTimeZhiIndex());
                },
                getTaiYuan: function () {
                ...
                getYun: function (gender, sect) {
                    ...
                    var dayGan = this.getDayGan(); // 通过闭包传递日干
                    var lunar = this.getLunar();
                    var yang = lunar.getYearGanIndexExact() % 2 === 0;
                    var man = gender === 1;
                    ...
                    var buildLiuYue = function (liuNian, index) {
                        return {
                            _p: { index: index, liuNian: liuNian },
                            getIndex: function () { return this._p.index; },
                    ...
                    var buildLiuNian = function (daYun, index) {
                        return {
                            _p: { year: daYun.getStartYear() + index, age: daYun.getStartAge() + index, index: index, daYun: daYun, lunar: daYun.getLunar() },
                            getYear: function () { return this._p.year; },
                            getAge: function () { return this._p.age; },
                            getIndex: function () { return this._p.index; },
                            getLunar: function () { return this._p.lunar; },
                            getGanZhi: function () {
                                var initialGanZhi = this._p.lunar.getJieQiTable()[I18n.getMessage('jq.liChun')].getLunar().getYearInGanZhiExact();
                                var offset = LunarUtil.getJiaZiIndex(initialGanZhi) + this._p.index;
                            ...
                            getGanZhiShiShen: function () {
                                var ganZhi = this.getGanZhi();
                                if (!ganZhi) return ['', []];
                                var gan = ganZhi.substring(0, 1);
                                var zhi = ganZhi.substring(1, 2);
                                var dayGanTemplate = LunarUtil.GAN_TO_TEMPLATE[dayGan];
                                var ganTemplate = LunarUtil.GAN_TO_TEMPLATE[gan];
                                var ganShiShenKey = dayGanTemplate + ganTemplate;
                                var ganShiShenTemplate = LunarUtil.SHI_SHEN[ganShiShenKey];
                                var ganShiShen = LunarUtil.SHI_SHEN_TEMPLATE_TO_SHORT[ganShiShenTemplate] || '';
                                var zhiShiShen = [];
                                var zhiTemplate = LunarUtil.ZHI_TO_TEMPLATE[zhi];
                                var hideGanTemplates = LunarUtil.ZHI_HIDE_GAN[zhiTemplate] || [];
                                for (var i = 0, j = hideGanTemplates.length; i < j; i++) {
                                    var hideGanShiShenKey = dayGanTemplate + hideGanTemplates[i];
                                    var hideGanShiShenTemplate = LunarUtil.SHI_SHEN[hideGanShiShenKey];
                                    var shortShiShen = LunarUtil.SHI_SHEN_TEMPLATE_TO_SHORT[hideGanShiShenTemplate];
                                    if (shortShiShen) zhiShiShen.push(shortShiShen);
                                }
                                return [ganShiShen, zhiShiShen];
                            },
                            getXun: function () { return LunarUtil.getXun(this.getGanZhi()); },
                            getXunKong: function () { return LunarUtil.getXunKong(this.getGanZhi()); },
                            getLiuYue: function () {
                                var l = [];
                                for (var i = 0; i < 12; i++) l.push(buildLiuYue(this, i));
                                return l;
                            }
                        };
                    };
                    ...
                    var buildXiaoYun = function (daYun, index, forward) {
                        return {
                            _p: { year: daYun.getStartYear() + index, age: daYun.getStartAge() + index, index: index, daYun: daYun, forward: forward, lunar: daYun.getLunar() },
                            getYear: function () { return this._p.year; },
                            getAge: function () { return this._p.age; },
                            getIndex: function () { return this._p.index; },
                            getGanZhi: function () {
                    ...
                    var buildDaYun = function (yun, index) {
                        ...
                        if (index < 1) {
                            startYear = birthYear; startAge = 1; endYear = year - 1; endAge = year - birthYear;
                        } else {
                            var add = (index - 1) * 10;
                            startYear = year + add; startAge = startYear - birthYear + 1; endYear = startYear + 9; endAge = startAge + 9;
                        }
                        return {
                            _p: { startYear: startYear, endYear: endYear, startAge: startAge, endAge: endAge, index: index, yun: yun, lunar: yun.getLunar() },
                            ...
                            getLiuNian: function (n) {
                                ...
                                var l = [];
                                for (var i = 0; i < n; i++) l.push(buildLiuNian(this, i));
                                return l;
                            },
                            getXiaoYun: function (n) {
                                ...
                                if (this._p.index < 1) n = this._p.endYear - this._p.startYear + 1;
                                var l = [];
                                for (var i = 0; i < n; i++) l.push(buildXiaoYun(this, i, this._p.yun.isForward()));
                                return l;
                            }
                        };
                    };
                    ...
                    return {
                        _p: { gender: gender, startYear: start.year, startMonth: start.month, startDay: start.day, startHour: start.hour, forward: forward, lunar: lunar },
                        ...
                        getDaYun: function (n) {
                            if (!n) n = 10;
                            var l = [];
                            for (var i = 0; i < n; i++) l.push(buildDaYun(this, i));
                            return l;
                        }
                    };
                },
        ...
        return {
            fromLunar: function (lunar, gender) {
                var instance = _fromLunar(lunar, gender);
                // 保持您原有的绑定逻辑
                if (instance._calculateShenSha) {
                    var boundCalculateShenSha = instance._calculateShenSha.bind(instance);
                    instance._calculateShenSha = boundCalculateShenSha;
                }
                return instance;
            }
        };
